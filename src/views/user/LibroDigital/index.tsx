...
import { IAuth } from '@/redux/authDuck';
...
const LibroDigital = ()=>{
    ...
    const auth: IAuth = useSelector((state: any) => state.auth.user)

    let fecha = new Date();
    let dia: string | number = fecha.getDate(); 
    let mes: string | number = fecha.getMonth() + 1; 
    let anio: string | number = fecha.getFullYear(); 
    ...
    let fechaPrimerDiaDelMes = anio + '-' + mes + '-' + '01';
    ...
    const generarLibroDigital = (): void => {
        setError({})
